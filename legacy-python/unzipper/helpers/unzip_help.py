...
async def progress_for_pyrogram(current, total, ud_type, message, start, unzip_bot):
    ...
    # Throttle updates: only edit every 5 seconds, or on final chunk
    last_update = getattr(message, "_last_progress_update", 0)
    is_done = (total > 0 and current >= total)
    if not is_done and (now - last_update) < 5:
        return
    ...
    if total == 0:
        ...
        eta_str = estimated_total_time if estimated_total_time != "" else "0 s"
...
async def progress_urls(current, total, ud_type, message, start):
    ...
    # Throttle: only update every 5 seconds or on completion
    last_update = getattr(message, "_last_progress_update", 0)
    is_done = (total > 0 and current >= total)
    if not is_done and (now - last_update) < 5:
        return
